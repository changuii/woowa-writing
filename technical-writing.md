...
### 요약 비교
| 방법 | 핵심 아이디어 | 장점 | 단점 | 본 사례 적용 |
|---|---|---|---|---|
| 넥스트키 락 | 행+앞 구간을 함께 잠급니다 | 트랜잭션 내부 자동 관리가 됩니다 / DB 데드락 탐지 혜택을 받습니다 | DB 자원 비용이 큽니다 / 범위로만 지정해 유연성이 낮습니다 / 확장 토폴로지 고려가 필요합니다 | 제한적입니다 |
| 네임드 락 | 문자열 키로 잠급니다 | 키를 임의로 설계할 수 있습니다 / 대기 시간을 지정할 수 있습니다 / DB 데드락 탐지를 활용합니다 | DB 자원 비용이 큽니다 / 샤딩, 레플리카 환경 고려가 필요합니다 | 가능하지만 비용 부담이 있습니다 |
| 분산락 | 외부 저장소로 상호배제를 관리합니다 | 키를 자유롭게 설계합니다 / DB를 사용하지 않습니다 / 수평 확장에 유리합니다 / 일반적으로 빠릅니다 | 인프라가 복잡해집니다 / 외부 저장소 장애 영향이 큽니다 / 임대 시간 설계가 필요합니다 | 적합합니다(저장소 선택이 과제입니다) |
...
실험 결과 핀 공지가 10개 생성되었습니다. 이는 HikariCP 최대 커넥션이 10개라 10개의 트랜잭션만 즉시 시작되고, 나머지 스레드는 커넥션 대기 상태에 머물러 더 이상 생성되지 못했기 때문입니다.
